...
export default function DrawingScreen() {
  ...
  const [mode, setMode] = useState<
    "draw" | "rect" | "square" | "circle" | "bucket"
  >("draw");
  ...
  type Shape =
    ...
    | {
      ...
      key: string;
      kind: "free";
      path: string;            // SVG path string generated by free draw
      strokeColor: string;
      strokeWidth: number;
    };
  ...
  const commitShape = () => {
    ...
    if (mode === "circle") {
      ...
      const { x, y, w, h } =
        mode === "square"
          ? getSquareFromPoints(shapeStart, shapeCurrent)
          : getNormalizedRect(shapeStart, shapeCurrent);
      setShapes((arr) =>
        arr.concat([
          {
            key,
  ...
  const onBucketGrant = (e: any) => {
    ...
    if (!region) {
      console.log("No region found at clicked point.", shapes, intersections);
      return;
    }

    if (region) {
      const svg = polygonToSvg(region[0]);
      console.log("Filling region with SVG:", svg);

      const prev = canvasRef.current?.toPaths?.() ?? [];

      const fillKey = `fill-${Date.now()}`;
      ...
      canvasRef.current?.drawPaths?.([
        ...prev,
        {
          ...
          fillColor, // your selected fill color
        },
      ]);
      setFilledColors((prev) => ({ ...prev, [fillKey]: fillColor }));
      console.log(fillColor, filledColors);
    }
  };
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={styles.controlsContainer}>
        ...
        <Text
          style={[styles.controlLabel, { color: colors.text, marginTop: 12 }]}
        >
          Width
        </Text>
      ...
      >
        <View style={[styles.canvasBorder, { borderColor: colors.tint }]}>
          <FreeCanvas
            ref={canvasRef}
            style={styles.canvas}
            background={Object.entries(filledColors).map(
              ([regionKey, color]) => {
                const regionPathObj = canvasRef.current
                  ?.toPaths?.()
                  .find((p: any) => p.key === regionKey);
                if (!regionPathObj) return null;

                return (
                  <SkiaPath
                    key={regionKey}
                    path={regionPathObj.path}
                    style="fill"
                    color={color}
                  />
                );
          ...
          {mode !== "draw" && mode !== "bucket" && (
            ...
            >
              ...
              {shapeStart &&
                ...
                (mode === "circle"
                  ? (() => {
                      const { x, y, w, h } = getNormalizedRect(
                        shapeStart,
                        shapeCurrent
                      );
      ...
      <View
        style={[
          styles.toolbar,
          { backgroundColor: colors.background, borderTopColor: colors.tint },
        ]}
      >
        <TouchableOpacity
